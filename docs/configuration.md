...
| `SESSION_KIT_SUBAGENT_IDLE_MINUTES` | `15` | minutes a sub-agent worker's own transcript must keep the same size and nanosecond modification time before the sweep sends TERM; the next pass sends KILL only if the same process remains. The five-minute timer normally closes a finished worker within about twenty minutes. `0` disables the sweep. An unreadable value disables it and says why. A long silent computation is indistinguishable from finished work, so raise this window if a live worker is ever swept. Exported variables do not reach the systemd timer; put the number in `<state>/subagent-sweep-minutes` for the timer-managed pass |
| `SESSION_KIT_SUBAGENT_IDLE_HOURS` | — | older name, used only when the minutes name is unset; `0` disables, and an unreadable value disables rather than guessing. The sweep remains Linux-only because exact signal-time PID proof requires live procfs. A reaper dry run previews actions only after real passes have armed the transcript clock, and writes nothing |
